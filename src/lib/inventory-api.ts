...
export const inventoryApi = {
  getInventory: async (
    ...
    if (params.locations?.length)
      formattedParams.locations = params.locations.join(",");
    if (params.parentIds?.length)
      formattedParams.parentIds = params.parentIds.join(",");
  ...
  duplicateInventoryItem: async (
    id: string,
    payload: DuplicateInventoryPayload,
  ): Promise<InventoryItem> => {
    return apiClient.post(`/inventory/${id}/duplicate`, payload);
  },
